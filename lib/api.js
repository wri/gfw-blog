...
export async function getPostsByTaxonomy({ params, allLanguages }) {
  let postsResponse;
  ...
  try {
    postsResponse = await apiFetch({
      ...
      url: `${process.env.NEXT_PUBLIC_WORDPRESS_URL}/wp/v2/taxonomy-filter`,
      params: {
        ...params,
        _embed: true,
        ...(!allLanguages && {
  ...
  return {
    page: params?.page || 1,
    posts: serializePosts(postsResponse?.data),
    totalPosts: parseInt(postsResponse?.headers['x-wp-total'], 10) || 0,
...
/*
 * Method to fetch WP Redirection plugin data
 * Reference: https://redirection.me/developer/rest-api/#api-Redirect-GetRedirects
 */
export async function getRedirectionData(slug) {
  const redirectionData = await apiFetch({
    url: `${process.env.NEXT_PUBLIC_WORDPRESS_URL}/redirection/v1/redirect?filterBy[target]=${slug}`,
  });

  const hasRedirection = redirectionData?.data?.total > 0;

  return hasRedirection ? redirectionData.data?.items[0] : {};
}
